  ...
  <div>
    <v-stepper v-model="step" alt-labels class="bgdarkopacity">
      ...
      <v-stepper-content :step="1">
        ...
        <v-btn @click="step++" depressed color="primary" class="d-block mx-auto"
          >Start
        </v-btn>
      </v-stepper-content>
    ...
    >
      <b
        ...
        ({{ CHEQD_CURRENT_AMOUNT_GIVEN }} {{ CHEQD_MINIMAL_DENOM }}).</b
      >
    </v-alert>
    <v-alert
      icon="mdi-shield-lock-outline"
    ...
    >
      <b
        >Address is not in the expected format for this chain or does not
        exist.</b
      >
    </v-alert>
    <v-alert
  ...
  data: () => {
    return {
      ...
      address_rules: [
        ...
        (value) =>
          /^(canow)1[a-z0-9]{38}$/.test(value) ||
          "Invalid cheqd address format.",
      ],
      CHEQD_MINIMAL_DENOM,
      CHEQD_CURRENT_AMOUNT_GIVEN,
      DEFAULT_TESTING_ADDRESS,
